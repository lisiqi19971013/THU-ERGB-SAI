...
from model_final.frame_net import MaskPredNet, PConvEncoder, PConvDecoder
from model_final import SizeAdapter


class FrameDeOccNet(nn.Module):
    def __init__(self, inChannels, img_channel=3, norm='BN'):
        super().__init__()
    ...
    def forward(self, img, mask_init):
        mask = self.MaskPredNet(img, mask_init)
        f1, m1 = self.Encoder(img, mask)
        output, opMask = self.FrameDecoder(f1, m1)
        return output, mask
...
if __name__ == '__main__':
    ...
    run_dir = './log/' + datetime.date.today().__str__() + '/frame_net'
    ...
    model = FrameDeOccNet(inChannels=51)
    ...
    for epoch in range(cur_epoch, epochs):
        ...
        for i, (event_vox, img, mask, gt) in enumerate(trainLoader):
            # event_vox = event_vox.cuda()
            img = img.cuda().float()
            mask = mask.cuda().float()
            gt_img = gt.cuda().float()

            output, m = model(img, mask)
        ...
        with torch.no_grad():
            ...
            ssim = 0
            count = 0
            for i, (event_vox, img, mask, gt) in enumerate(testLoader):
                # event_vox = event_vox.cuda()
                img = img.cuda().float()
                mask = mask.cuda().float()
                gt_img = gt.cuda().float()
        ...
        model_dict = {'epoch': epoch, 'MaskPredNet': model.module.MaskPredNet.state_dict(),
                      "Encoder":model.module.Encoder.state_dict(),
                      "FrameDecoder":model.module.FrameDecoder.state_dict(),
                      }
